...
// below build meet error,so change _1 to __1, _2 to __2;
namespace {
  class placeholder {};
  placeholder __1;
  placeholder __2;
...
public:
  simple_bind_t1(fn f, const T& t) : fn_(f), t_(t) {}

  R operator()(Arg& a) { return (t_.*fn_)(a); }
};

template <typename R, typename T, typename Arg>
simple_bind_t1<R, T, Arg> simple_bind(R(T::*fn)(Arg),
...
bool test_bind() {
  ...
  // //等同于下面2行代码逻辑
  boost::function<void(int&, int&)> f(simple_bind(&test_bind_class::do_something, &(*ptr_test), __1, __2));

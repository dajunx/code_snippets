...
void handle_net_epoll_client(int connfd);

bool test_net_epoll_client(int argc, char **argv) {
  ...
  handle_net_epoll_client(connfd); /* do it all */
  close(connfd);
  printf("exit\n");
  exit(0);
...
void handle_net_epoll_client(int sockfd) {
